...
class BakiyeController extends Controller
{
    ...
    {
        $pdo = Database::pdo();
        $records = Bakiye::all($pdo);

        $this->view('bakiye/index', [
            'title' => 'Bakiye',
            'records' => $records,
        ], 'layouts/base');
    }
    ...
    public function apiRecords(): void
    {
        ...
        try {
            ...
            // Get total count
            $countStmt = $pdo->query("SELECT COUNT(*) as count FROM bakiye");
            ...
            $sql = "SELECT * FROM bakiye ORDER BY id DESC LIMIT :limit OFFSET :offset";
    ...
    {
        $this->view('bakiye/create', [
            'title' => 'Bakiye Kaydı Ekle',
        ]);
    }
    ...
    public function store(): void
    {
        ...
        try {
            ...
            Bakiye::create($pdo, $data);

            header('Location: /bakiye', true, 302);
            exit;
        } catch (\Exception $e) {
    ...
    public function edit(): void
    {
        ...
        $this->view('bakiye/edit', [
            'title' => 'Bakiye Kaydı Düzenle',
            'record' => $record,
        ]);
    }
    ...
    public function bulkUpload(): void
    {
        ...
        try {
            error_log('[Bakiye BulkUpload] Starting...');
            ...
            if (!$payloadJson) {
                ...
                ob_end_flush();
                error_log('[Bakiye BulkUpload] Error: payload missing');
                return;
            }
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                ...
                ob_end_flush();
                error_log('[Bakiye BulkUpload] Error: invalid JSON - ' . json_last_error_msg());
                return;
            }
            ...
            if (!is_array($rows) || count($rows) < 2) {
                ...
                ob_end_flush();
                error_log('[Bakiye BulkUpload] Error: rows invalid');
                return;
            }

            error_log('[Bakiye BulkUpload] Received ' . count($rows) . ' rows');
            ...
            if (!empty($errors)) {
                ...
                ob_end_flush();
                error_log('[Bakiye BulkUpload] Partial success: ' . $inserted . ' records, errors: ' . count($errors));
                return;
            }
            ...
            echo json_encode([
                'inserted' => $inserted,
                'message' => "$inserted bakiye kaydı başarıyla yüklendi"
            ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            ob_end_flush();
            error_log('[Bakiye BulkUpload] Success: inserted ' . $inserted . ' records');
        } catch (\Exception $e) {
            http_response_code(500);
            echo json_encode(['message' => 'Exception: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            ob_end_flush();
            error_log('[Bakiye BulkUpload] Exception: ' . $e->getMessage());
        }
    }
}
