...
$title = $title ?? trans('labels.app_name');
...
<body>
  <nav class="navbar navbar-expand-lg bg-white border-bottom sticky-top">
    <div class="container-fluid">
      <a class="navbar-brand fw-semibold text-primary" href="/"><?= trans('common.app_name') ?></a>
      ...
      <div class="collapse navbar-collapse" id="topnav">
        <ul class="navbar-nav me-auto">
          <li class="nav-item"><a class="nav-link" href="/attendance/report"><?= trans('labels.attendance') ?></a></li>
          <li class="nav-item"><a class="nav-link" href="/firms"><?= trans('labels.firms') ?></a></li>
        </ul>
        <div class="d-flex gap-2 align-items-center">
          <!-- Language Switcher -->
          <div class="dropdown">
            <button class="btn btn-outline-secondary btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
              <i class="bi bi-globe me-1"></i><?= strtoupper(currentLanguage()) ?>
            </button>
            <ul class="dropdown-menu dropdown-menu-end">
              <?php foreach (getLanguages() as $code => $name): ?>
                <?php if (isLanguage($code)): ?>
                  <li><span class="dropdown-item active"><?= $name ?></span></li>
                <?php else: ?>
          ...
          <a class="btn btn-outline-secondary btn-sm" href="/logout"><i class="bi bi-box-arrow-right me-1"></i><?= trans('common.logout') ?></a>
        </div>
      </div>
    </div>
  </nav>
