...
<div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
  ...
  <div class="d-flex flex-wrap gap-2">
    <a class="btn btn-primary" href="/projects/create"><i class="bi bi-plus-lg me-1"></i><?= trans('common.new_project') ?></a>
...
<script>
  window.LANG = {
    ...
    'short_name': 'Kısa Ad',
    'path': 'Path',
    'firm_id': '<?= trans('labels.firm_id') ?>',
    'start_date': '<?= trans('labels.start_date') ?>',
    'end_date': '<?= trans('labels.end_date') ?>',
...
<script>
  (function() {
    ...
    const allFields = [{
      ...
      {
        id: 'created_at',
        label: getLang('created_at')
      },
      {
        id: 'updated_at',
        label: getLang('updated_at')
      },
      {
        id: 'deleted_at',
        label: getLang('deleted_at')
      },
      {
        id: 'name',
        label: getLang('name')
      ...
      {
        id: 'timezone',
        label: getLang('timezone')
      },
      {
        id: 'image_url',
        label: getLang('image_url')
      },
      {
        id: 'notes',
        label: getLang('notes')
      ...
      {
        id: 'address_line1',
        label: getLang('address') + ' 1'
      },
      {
        id: 'address_line2',
        label: getLang('address') + ' 2'
      },
      {
        id: 'city',
        label: getLang('city')
      ...
      {
        id: 'created_by',
        label: getLang('created_by')
      },
      {
        id: 'updated_by',
        label: getLang('updated_by')
      },
    ];
    ...
    // Storage helper - safe wrapper around localStorage
    ...
    // UI events - check if elements exist before adding listeners
    const toggleBtn = document.getElementById('toggleColumnPanel');
    if (toggleBtn) toggleBtn.addEventListener('click', () => columnPanel.hidden = !columnPanel.hidden);
    ...
    } else if (table && tbody && headerRow && filtersRow) {
      buildColumnPanel();
      rebuildHeadAndCols();
      render();
    } else {
    ...
    function loadVisibleCols() {
      try {
        const raw = getStorage(LS_KEYS.visibleCols);
    ...
    function saveFiltersToStorage() {
      ...
      setStorage(LS_KEYS.filters, JSON.stringify(state.filters));
    }

    function loadSort() {
      try {
        const s = JSON.parse(getStorage(LS_KEYS.sort, '{"by":"id","dir":"asc"}'));
    ...
    function loadWidths() {
      try {
        return JSON.parse(getStorage(LS_KEYS.widths, '{}')) || {};
      } catch {
        return {};
      }
    }
    ...
    function loadPage() {
      try {
        ...
        const p = Number(getStorage(LS_KEYS.page, '1'));
    ...
    function loadLimit() {
      try {
        ...
        if (!Number.isNaN(fromQs) && fromQs > 0) return fromQs;
        const l = Number(getStorage(LS_KEYS.limit, '50'));
    ...
    function saveAll() {
      setStorage(LS_KEYS.visibleCols, JSON.stringify(state.visibleCols));
      saveFiltersToStorage(); // Use the new storage function
      setStorage(LS_KEYS.sort, JSON.stringify(state.sort));
      setStorage(LS_KEYS.widths, JSON.stringify(state.widths));
      savePageAndLimit();
    }

    function savePageAndLimit() {
      setStorage(LS_KEYS.page, String(state.page));
      setStorage(LS_KEYS.limit, String(state.limit));
    }
    ...
    function normalizeTableStructure() {
      ...
      if (headerRow && headerRow.parentNode !== th) th.appendChild(headerRow);
      if (filtersRow && filtersRow.parentNode !== th) th.insertBefore(filtersRow, th.firstChild);
...
`;
    ...
    function rebuildHeadAndCols() {
      if (!table || !headerRow || !filtersRow || !colGroup) return;
