...
<div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
    <h1 class="h4 m-0"><?= Helpers::e($title ?? 'Bakiye') ?></h1>
    <div class="d-flex flex-wrap gap-2">
        <a class="btn btn-primary" href="/bakiye/create"><i class="bi bi-plus-lg me-1"></i>Yeni Kayıt</a>
    ...
    <div class="card-body p-0">
        <div class="table-wrap p-2 pt-0">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" id="bakiyeTable">
...
<style>
    ...
    #bakiyeTable thead th {
        position: relative;
        background-clip: padding-box;
        white-space: nowrap;
    }

    #bakiyeTable thead tr#filtersRow th {
        padding: .25rem .5rem;
        border-bottom: 0 !important;
    }

    #bakiyeTable thead tr#headerRow th {
        padding-top: .25rem;
        padding-bottom: .4rem;
    ...
    #bakiyeTable thead input.form-control-sm,
    #bakiyeTable thead select.form-select-sm {
        min-height: 32px;
        line-height: 1.2;
    }

    #bakiyeTable th.col-actions {
        padding-left: .25rem;
        padding-right: .25rem;
    }

    #bakiyeTable th .col-resizer {
        position: absolute;
        top: 0;
        right: 0;
        width: 10px;
    ...
    #bakiyeTable th.resizing,
    #bakiyeTable th .col-resizer.active {
    ...
    #bakiyeTable td .btn-group {
        gap: 4px;
    }

    #bakiyeTable td .btn-group .btn {
        border-width: 1px;
    }

    #bakiyeTable thead th.sortable {
        cursor: pointer;
    }

    #bakiyeTable thead th.sortable[data-sort="asc"]::after {
        content: " ↑";
        opacity: .6;
    }

    #bakiyeTable thead th.sortable[data-sort="desc"]::after {
        content: " ↓";
        opacity: .6;
    }
...
<script>
    ...
    async function initBakiyeTable() {
        // Load data via paginated API to avoid memory issues
        window.DATA = [];
        try {
            let page = 1;
            ...
            while (hasMorePages) {
                const resp = await fetch(`/bakiye/api/records?page=${page}&pageSize=${pageSize}`, {
                    method: 'GET',
                    headers: {
                ...
                if (resp.ok) {
                    ...
                    if (result.data && Array.isArray(result.data)) {
                        ...
                        console.log(`Loaded page ${page}: ${result.data.length} records (Total: ${DATA.length}/${result.pagination.total})`);
            ...
            console.log('Loaded all bakiye records:', window.DATA.length);
        } catch (e) {
            console.warn('Could not load bakiye records via API:', e);
            window.DATA = [];
        }
        ...
        const pageSize = parseInt(localStorage.getItem('bakiye_pageSize')) || 20;

        let visibleFields = JSON.parse(localStorage.getItem('bakiye_visibleFields')) || defaultVisibleFields;
        let currentPage = 1;
        let sortField = null;
        let sortDir = 'asc';
        window.filters = {};
        ...
        window.renderTable = function renderTable() {
            ...
            paged.forEach(record => {
                ...
                actionsCell.innerHTML = `
                    <a href="/bakiye/edit?id=${record.id}" class="btn btn-sm btn-warning btn-icon" title="Düzenle">
            ...
            if (window.updateTotals_bakiyeTable) {
                window.updateTotals_bakiyeTable([]);
            }
        }
        ...
        pageSizeSelect.onchange = () => {
            localStorage.setItem('bakiye_pageSize', pageSizeSelect.value);
            currentPage = 1;
            renderTable();
        };

        resetButton.onclick = () => {
            localStorage.removeItem('bakiye_visibleFields');
        ...
        function renderColumnCheckboxes() {
            ...
            allFields.forEach(field => {
                ...
                checkbox.onchange = () => {
                    ...
                    localStorage.setItem('bakiye_visibleFields', JSON.stringify(visibleFields));
                    renderTable();
                };
        ...
        setTimeout(() => {
            ...
            renderColumnCheckboxes();
            // Initialize totals FIRST so window.updateTotals_bakiyeTable exists
            initializeBakiyeTotals();
            // Then render the table (which will call updateTotals)
            renderTable();
        }, 100);
    }

    function initializeBakiyeTotals() {
        initTableTotals({
            tableId: 'bakiyeTable',
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            initBakiyeTable();
            setTimeout(initializeBakiyeTotals, 150);
        });
    } else {
        initBakiyeTable();
        setTimeout(initializeBakiyeTotals, 150);
    ...
    async function downloadTemplateXlsx() {
        ...
        XLSX.writeFile(wb, 'bakiye_template.xlsx');
    }
    ...
    function exportToExcelXlsx() {
        ...
        const ws = XLSX.utils.aoa_to_sheet(wsData);
        XLSX.utils.book_append_sheet(wb, ws, 'Bakiye');
        XLSX.writeFile(wb, 'bakiye_export_' + dateStr + '.xlsx');
    }
    ...
    async function handleConfirmUpload(e) {
        ...
        try {
            const resp = await fetch('/bakiye/bulk-upload', {
                method: 'POST',
                headers,
                body: formData,
                credentials: 'same-origin'
            ...
            alert('Yükleme başarılı: ' + (data?.inserted || 0) + ' bakiye kaydı eklendi.');
