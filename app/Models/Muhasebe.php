...
class Muhasebe
{
    // List all muhasebe records
    public static function all(PDO $pdo): array
    {
        $sql = "SELECT * FROM muhasebe ORDER BY id DESC";
    ...
    {
        $stmt = $pdo->prepare("SELECT * FROM muhasebe WHERE id = :id LIMIT 1");
        $stmt->execute([':id' => $id]);
    ...
    {
        $stmt = $pdo->prepare("DELETE FROM muhasebe WHERE id = :id");
