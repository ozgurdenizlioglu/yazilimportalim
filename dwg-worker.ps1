        ...
        foreach ($dwgFile in $dwgFiles) {
            ...
            try {
                ...
                try {
                    ...
                    Write-Log "  Warning - could not save workbook - $err"
                }
                # Find METRAJ sheet and export table data
                $worksheet = $null
                Write-Log "  Searching for tbl_Metraj..."
                foreach ($ws in $workbook.Worksheets) {
                    Write-Log "    Checking sheet: $($ws.Name)"
                    try {
                        if ($ws.ListObjects("tbl_Metraj")) {
                            $worksheet = $ws
                            Write-Log "    Found tbl_Metraj!"
                            break
                        }
                    }
                    catch {
                        $err = $_.Exception.Message
                        Write-Log "    Error checking sheet - $err"
                    }
                }
                
                $itemsCount = 0
                if ($worksheet) {
                    Write-Log "  Processing table from sheet: $($worksheet.Name)"
                    ...
                    if ($dataRange) {
                        ...
                        if ($rows -gt 0) {
                            try {
                                ...
                                Write-Log "  Columns: $cols"
                                Write-Log "  Starting to build items..."
                                $items = @()
                                    ...
                                    try {
                                        ...
                                        for ($c = 1; $c -le $cols; $c++) {
                                            try {
                                                ...
                                                $err = $_.Exception.Message
                                                Write-Log "    ERROR at row $r col $c - $err"
                                            }
                                        }
                                        $items += $row
                                    }
                                        ...
                                        $err = $_.Exception.Message
                                        Write-Log "    ERROR processing row $r - $err"
                                    }
                                    if (($r % 50) -eq 0) { Write-Log "  Processed $r / $rows rows" }
                                }
                                
                                Write-Log "  Built $($items.Count) item objects"
                            }
                            catch {
                                $err = $_.Exception.Message
                                Write-Log "  ERROR building items - $err"
                            }
                            ...
                            try {
                                ...
                                Write-Log "  JSON body size: $($body.Length) bytes"
                                Write-Log "  Posting $($items.Count) items to API..."
                                ...
                                try {
                                    ...
                                    Write-Log "  API Response JSON: $respJson"
                                    $imported = $null
                                    $total = $null
                                    if ($response -is [string]) {
                                        Write-Log "  API returned string response"
                                    }
                                    else {
                                        if ($response.PSObject.Properties.Name -contains 'imported') { $imported = $response.imported }
                                        if ($response.PSObject.Properties.Name -contains 'total') { $total = $response.total }
                                    }
                                    Write-Log "  API Parsed: imported=$imported, total=$total"
                                }
                                catch {
                                    $err = $_.Exception.Message
                                    Write-Log "  Warning - could not parse API response - $err"
                                }
                            }
                            catch {
                                $err = $_.Exception.Message
                                Write-Log "  ERROR posting to API - $err"
                            }
                        }
                        else {
